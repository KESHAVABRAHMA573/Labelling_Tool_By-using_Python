...
class Load_file:
    def __init__(self):
        ...
        self.NumberShowing.bind("<Return>", lambda e: (self.jump_to_index(), self.root.focus_set()))
        ...
        self.save.grid(row=2, column=5, padx=10, pady=10)
        ...
        self.Edit = tk.Button(self.bottom_frame, text="Edit", command=self.editPressed)
        self.Edit.grid(row=2, column=7, padx=10, pady=10)
        ...
        try:
            ...
            if self.ImagePath:
                ...
                for i, path in enumerate(self.ImagePath):
                    ...
                    if os.path.exists(json_path) or os.path.exists(dat_path):
    ...
    def jump_to_index(self, event=None):
        try:
            raw = self.NumberShowing.get().strip()
            if "/" in raw:
                raw = raw.split("/")[0]  # take only the part before the slash
            index = int(raw) - 1
            if 0 <= index < len(self.ImagePath):
                self.CurrentIndex = index
                self.showImagePaths()
            else:
                messagebox.showwarning("Invalid Index", f"Enter a number between 1 and {len(self.ImagePath)}")
        except ValueError:
...
#-----------------------------------------------------------------------------------------------------------
    ...
    def confirm_save_before_switch(self, direction):
        if self.shapes_modified:
            ...
            # Reload original shapes from JSON
                path = self.ImagePath[self.CurrentIndex]
                label_path = os.path.splitext(path)[0] + "_label.json"
                self.new_shapes.clear()
                self.temp_shapes.clear()
    ...
    def enable_polygon_deletion(self):
        ...
        def on_click(event):
            ...
            for item in reversed(overlapping):
                if item in self.polygon_id:
                    index = self.polygon_id.index(item)
                    self.edit_undo_stack.append({"action": "delete_polygon",
                    ...
                    self.redraw_canvas()
                    self.new_shapes = [shape for i, shape in enumerate(self.new_shapes)
                    if i != index]
                    break
    ...
    def start_zoom_rect(self, event):
        ...
        self.zoom_rect_id = self.canvas.create_rectangle(*self.zoom_start, *self.zoom_start, outline="blue", dash=(4, 2))
    ...
    def show_dat_file(self, event=None):
        ...
        msk_path = os.path.splitext(image_path)[0] + "_label.dat"
        if not os.path.exists(msk_path):
            messagebox.showerror("Missing", "No Dat file found for current image")
            return
    ...
    def savePressed(self, force=False):
        ...
        if not self.shapes_modified and not force and not format_changed:
